"""
Think-on-Graph (ToG) Worker Implementation for KG Library.
"""
...
class ToGWorker:
    ...
    def execute_query(
        ...
        # Build a per-request policy copy so we never mutate shared gateway state
        policy_overrides: dict = {}
        if ai_enabled is not None:
            policy_overrides["ai_enabled"] = ai_enabled
        if max_ai_calls is not None:
            policy_overrides["max_ai_calls"] = max_ai_calls
        if traversal_mode in ("manual", "ai", "hybrid"):
            policy_overrides["traversal_mode"] = traversal_mode

        request_policy = self.ai_gateway.policy.model_copy(update=policy_overrides) if policy_overrides else self.ai_gateway.policy
        request_escalation_gate = EscalationGate(request_policy)
        ...
        # Step 4: Escalation Check (use per-request policy copy)
        escalate = request_escalation_gate.should_escalate(query, search_res)
