"""
Storage abstraction and in-memory implementation for KG Library Knowledge Graph.
"""
...
class BaseStorageEngine(ABC):
    ...
    @abstractmethod
    def list_relationships(self, skip: int = 0, limit: int = 100) -> List[Relationship]:
        pass
...
class InMemoryStorageEngine(BaseStorageEngine):
    """Thread-safe in-memory storage engine implementation."""
    ...
    def list_nodes(self, skip: int = 0, limit: int = 100) -> List[Node]:
        with self._lock:
            return list(self.nodes.values())[skip: skip + limit]

    def list_relationships(self, skip: int = 0, limit: int = 100) -> List[Relationship]:
        with self._lock:
            return list(self.relationships.values())[skip: skip + limit]
...
from kg_library_api.core.schema import Base, SQLNode, SQLRelationship, SQLCollection
...
class SQLStorageEngine(BaseStorageEngine):
    ...
    def list_nodes(self, skip: int = 0, limit: int = 100) -> List[Node]:
        with self.Session() as session:
            rows = session.query(SQLNode).offset(skip).limit(limit).all()
            return [Node(id=r.id, label=r.label, properties=r.properties or {}) for r in rows]

    def list_relationships(self, skip: int = 0, limit: int = 100) -> List[Relationship]:
        with self.Session() as session:
            rows = session.query(SQLRelationship).offset(skip).limit(limit).all()
            return [
                Relationship(
                    id=r.id, source_id=r.source_id, target_id=r.target_id,
                    type=r.type, properties=r.properties or {}, directed=r.directed
                )
                for r in rows
            ]
    ...
    def delete_relationship(self, relationship_id: str) -> bool:
        with self.Session() as session:
            r = session.query(SQLRelationship).filter(SQLRelationship.id == relationship_id).first()
            if not r:
                return False
            session.delete(r)
            session.commit()
