...
from polaris_kg.retrieval.retriever import MultiPerspectiveSearchResult
from polaris_kg.ai.gateway import AIGateway
from polaris_kg.ai.schemas import AIReasonRequest
...
class ToGSynthesizer:
    ...
    def synthesize(
        ...
        if escalate and self.ai_gateway.policy.ai_enabled:
            system_prompt = (
                "You are Polaris, a hybrid graph reasoning agent. Format your response clearly "
            ...
            if ai_res.model == "fallback":
                ...
                if not parts:
                    ...
                    answer = f"Budget limit exhausted for query session. Based on knowledge graph traversal:\n" + "\n".join(f"- {p}" for p in parts)
            ...
            if not parts:
                ...
                answer = f"Based on knowledge graph traversal:\n" + "\n".join(f"- {p}" for p in parts)
